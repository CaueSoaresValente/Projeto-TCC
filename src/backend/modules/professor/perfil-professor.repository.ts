...
export class PerfilProfessorRepository {
  ...
  private oppAreaRepo = AppDataSource.getRepository(OPPArea);
  private profAreaRepo = AppDataSource.getRepository(ProfessorArea);
  ...
  async findByOPPAreas(idOPP: number): Promise<Professor[]> {
  ...
  async findFullProfile(idProfessor: number): Promise<Professor | null> {
  ...
  async findOPPAreas(idOPP: number): Promise<OPPArea[]> {
